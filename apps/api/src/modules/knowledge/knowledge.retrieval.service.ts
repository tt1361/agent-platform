...
export class KnowledgeRetrievalService {
  async retrieve(query: string, limit: number = 5, executionId?: string) {
  ...
  async listCitedByAnswer(items: KnowledgeRetrievalResultItem[], answerText?: string | null) {
    return filterCitedItems(items, answerText);
  }
}
