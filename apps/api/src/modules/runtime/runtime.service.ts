...
import { ExecutionTracer } from '../../common/execution-tracer.js';
import { getLangchainTool } from '../../llm/skill-discovery.js';
...
export type AgentRunEvent =
  ...
  | { type: 'retrievals'; executionId: string; traceId: string; items: Awaited<any> }
  | { type: 'trace_step'; step: AgentTraceStreamStep }
  | { type: 'answer_start'; executionId: string; traceId: string }
  | { type: 'completed'; result: Awaited<any> }
...
export class RuntimeService {
  ...
  async run(params: RunAgentParams) {
    ...
    await this.conversationService.touch(activeConversationId);
    ...
    const [latestShortTermMemory, longTermMemories, retrievedKnowledge] = await Promise.all([
      this.memoryService.getLatestShortTermMemory(activeConversationId),
      this.memoryService.getLongTermMemories(agent.id, params.input),
      this.knowledgeRetrievalService.retrieve(params.input, 8, execution.id),
    ]);
...
${item.content}`
  ...
  .join('\n\n---\n\n')}`
    ...
    const activeFallbackProviders = await prisma.llmProvider.findMany({
      where: {
        status: 'active',
        providerType: {
          in: ['qwen', 'minimax'],
    ...
    const fallbackProvider =
      activeFallbackProviders.find((item) => item.providerType === 'qwen') ?? activeFallbackProviders[0] ?? null;

    const buildModel = (providerType: 'qwen' | 'gemini' | 'minimax') => {
      let nextModel: any;
      if (providerType === 'qwen') {
        nextModel = new ChatQwen();
      } else if (providerType === 'gemini') {
        nextModel = new ChatGemini();
      } else {
        nextModel = new ChatMiniMax();
      }

      if (nextModel.bindTools) {
        nextModel = nextModel.bindTools(langchainTools);
      }

      return nextModel;
    };

    let activeProviderType = (agent.llmProvider?.providerType ?? 'minimax') as 'qwen' | 'gemini' | 'minimax';
    let model: any = buildModel(activeProviderType);
    ...
    const callModel = async (state: typeof GraphState.State) => {
      ...
      try {
        ...
        if (activeProviderType === 'gemini' && isGeminiRegionUnsupportedError(error)) {
          if (!fallbackProvider) {
            throw new HttpError(400, 'PROVIDER_UNAVAILABLE', 'Gemini 在当前地区不可用，且未找到可用的备用模型（Qwen/MiniMax）');
          }
          ...
          await this.recordTraceStep(params, {
            ...
            stepType: 'observation',
            content: `Gemini 在当前地区不可用，已自动切换到 ${fallbackProvider.name}`,
          });

          activeProviderType = fallbackProvider.providerType as 'qwen' | 'gemini' | 'minimax';
          model = buildModel(activeProviderType);
          await prisma.execution.update({
            where: { id: execution.id },
            data: { providerId: fallbackProvider.id },
          });
          response = await model.invoke(state.messages);
        } else {
          throw error;
    ...
    try {
      ...
      const citedKnowledgeRetrievals = await this.knowledgeRetrievalService.listCitedByAnswer(retrievedKnowledge, answer);
      ...
      const shortTermMemory = await this.memoryService.updateShortTermMemory({
        conversationId: activeConversationId,
        conversationTitle,
      ...
      const updatedLongTermMemories = await this.memoryService.persistLongTermMemories({
        agentId: agent.id,
        conversationId: activeConversationId,
        input: params.input,
        output: answer,
